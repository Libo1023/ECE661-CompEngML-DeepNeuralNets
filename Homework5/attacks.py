...
def PGD_attack(model, device, dat, lbl, eps, alpha, iters, rand_start):
    ...
    for i in range(0, iters) :
        ...
        x_adv = x_adv.clone().detach() + alpha * torch.sign(gradient).to(device)

        # Clip the perturbed datapoints to ensure we still satisfy L_infinity constraint
        eps_constraint = torch.clamp(x_adv.clone().detach() - x_nat.clone().detach(), (-1)*eps, eps)
        
        # Clip the perturbed datapoints to ensure we are in bounds [0,1]
        x_adv = torch.clamp(x_nat.clone().detach() + eps_constraint.clone().detach(), 0., 1.)
...
def FGSM_attack(model, device, dat, lbl, eps):
    ...
    # rand_start should be False for FGSM.
    # Advice acquired from Lead TA during OH.
...
def FGM_L2_attack(model, device, dat, lbl, eps):
    ...
    gradient_flatten = torch.flatten(gradient.clone().detach(), start_dim = 1)
    l2_of_grad = torch.sqrt(torch.sum(gradient_flatten * gradient_flatten, 1))
    
    # Perturb the data using the gradient
    # HINT: Before normalizing the gradient by its L2 norm, use
    # torch.clamp(l2_of_grad, min=1e-12) to prevent division by 0
    l2_of_grad = torch.clamp(l2_of_grad.clone().detach(), min = 1e-12)
    
    # Add perturbation the data
    for i in range(gradient.shape[0]) :
        for j in range(gradient.shape[2]) :
            for k in range(gradient.shape[3]) :
                gradient[i][0][j][k] = gradient[i][0][j][k] / l2_of_grad[i]
